...
import { buscarReview, removerFilme, type Review } from "../services/reviews"
...
export const MeusFilmes = () => {
    ...
    return (
        ...
        <div className="p-8">
            ...
            <div className="grid grid-cols-4 gap-6">
                {filmes?.map((filme) => (
                    ...
                    <div key={filme.id_filme}
                        onClick={async () => {await buscarFilmeReview(usuario?.id_user as number, filme.id_filme)
            ...
            {showReview && ( 
                <div onClick={() => setShowReview(false)}className="fixed inset-0 flex items-center justify-center z-50">
                    <div onClick={(e) => e.stopPropagation()}className="bg-white rounded-2xl p-6 w-full max-w-lg">
                        <h3 className="text-xl font-bold mb-6 text-center">{filme?.titulo}</h3>
                        <div className="mb-4">
                            <label className="text-sm font-bold text-gray-600 block mb-2text-sm font-bold text-gray-600 block mb-2">Sua nota: {review?.[0]?.nota}</label>
                        </div>
                        <div className="mb-6">
                            <label className="text-sm font-bold text-gray-600 block mb-2">Sua review:</label>
                            <textarea className="border border-gray-300 rounded-lg p-3 w-full resize-none" readOnly value={review?.[0]?.review} rows={4}></textarea>
                        </div>
                        <div className="flex gap-3">
                            <button type="button" className="bg-blue-600 text-white rounded-lg p-3 flex-1 hover:bg-blue-700">
                            Editar
                            </button>
                            <button type="button" onClick={ async () => {await remover(review?.[0]?.id_review as number, filme?.id_filme as number)
                                setShowReview(false)
                                setFilmes(filmes?.filter(f => f.id_filme !== filme?.id_filme) ?? [])
